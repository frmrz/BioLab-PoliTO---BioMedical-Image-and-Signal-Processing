...
import torch
from Classes import Dataset, Dataset_test, visualize
...
def get_training_augmentation(p_aug=1):
    ...
    if p_transform < p_aug:
        train_transform = [
            ...
            albu.augmentations.geometric.transforms.OpticalDistortion (distort_limit=0.05, shift_limit=0.05, interpolation=1, border_mode=4, value=None, mask_value=None, always_apply=False, p=0.2),                   
            albu.OneOf(
                [
...
if __name__ == '__main__':
    ...
    # for rr in range(0,8): ### REPETITIONS
    for kk in range(0,5):    ### FOLDS
    
        # print(kk)
        DATA_DIR = '/media/francesco/DEV001/PROJECT-GITHUB/Image_Processing/Segmentation/Ultrasound-OpticNerveSheaths/DATA/Paper_CV_folds/f{}'.format(kk)
        
        # Experiment_name = 'Unet-resnet50-f{}-r{}'.format(kk,rr)
        Experiment_name = 'OpticNerveSegmentation-f{}'.format(kk)
        ...
        CLASSES         = ['background','ons','on'] # labels 0, 1, 2 
        ...
        model = smp.Unet('resnet18', classes=3,
        ...
        patience_train = 0
        num_epochs_decay = 10
        a=next(iter(train_loader)) # check if it works
        ...
        for i in range(0, 50):
            ...
            if valid_logs[loss.__name__] < (min_loss - 0.01*min_loss):
        ...
        # best_model = torch.load('/home/francesco/PROJECT-OPTIC-NERVE/CODE/Segmentation_models/checkpoints-resnet50-3classes/Unet-resnet18-f0-r3/best_model.pth')

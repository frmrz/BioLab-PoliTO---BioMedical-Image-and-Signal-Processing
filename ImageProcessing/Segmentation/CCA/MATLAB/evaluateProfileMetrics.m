...
toCompare = [4,6,8,10,22,23,24,26];

for k = 1 : length(toCompare)
    ii = toCompare(k);
    tempFolder = fullfile(profilesDir,folders(ii).name);
    
%     GTFolder = fullfile(profilesDir,'GT1-ORIGINAL');
%     intGTFolder = fullfile(profilesDir,'GT1-INTERPOLATED');
...
%                 MA_GT = TurnColumn(load(fullfile(GTFolder,strrep(fxs(jj).name,'.png','-MA.txt'))));
                ...
                %% compare with current profiles
                    ...
                    if ~contains(tempFolder,'ORIGINAL') && lenGT > 1 && percentOverlapPred > 0.1
                        ...
                        comm.temp_LI = LI(ismember(round(LI(:,1)),round(LI_GT_interp(:,1))),:);               comm.temp_MA = MA(ismember(round(MA(:,1)),round(MA_GT_interp(:,1))),:);
                        comm.gt_LI = LI_GT_interp(ismember(round(LI_GT_interp(:,1)),round(LI(:,1))),:);       comm.gt_MA = MA_GT_interp(ismember(round(MA_GT_interp(:,1)),round(MA(:,1))),:);
...
save('/media/francesco/FMZ_archive/PROJECT-CUBS-SPIE/RESULTS/MATLAB/profileEvaluationConsensus-INTER-biasProfiles.mat', ...
